...
/*DISPLAY CURRENT TIME ,SPEED, EVENT*/
...
char login(void)
{
    ...
    /*READING 4-DIGIT PASSWORD FROM KEYBOARD*/
    while(index < 4)    
    {
    ...
    /*COMPARING USER-ENTERED PASSWORD AND ACTUAL PASSWORD*/
    ...
    /*IF SYSTEM EXCEEDS LIMIT*/
    if(attempts >= 5)
    {
        clcd_print(" SYSTEM LOCKED ", LINE1(0));
        clcd_print(" WAIT 120 SEC  ", LINE2(0));    
    ...
    clcd_putch((5 - attempts) + '0', LINE2(7));   
...
/* MODIFY RTC TIME AND SAVE TO DS1307*/
void set_time(void)
{
